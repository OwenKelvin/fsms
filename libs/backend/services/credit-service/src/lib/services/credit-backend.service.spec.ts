...
describe('CreditBackendService', () => {
  let service: CreditBackendService;

  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CreditBackendService,
        {
          provide: getModelToken(CreditModel),
          useValue: modelRepositoryMock,
        },
      ],
    }).compile();

    service = module.get<CreditBackendService>(CreditBackendService);
  });
