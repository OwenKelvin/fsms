...
describe('ActivityLogBackendService', () => {
  let service: ActivityLogBackendService;
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getModelToken(ActivityLogModel),
          useValue: modelRepositoryMock,
        },
      ],
    }).compile();

    service = module.get<ActivityLogBackendService>(ActivityLogBackendService);
  });
