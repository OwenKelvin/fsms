...
import { PlanInfoBackendService } from './plan-info-backend.service';
import {
  QueryOperatorEnum,
  PlanInfoModel,
  SortByDirectionEnum,
...
describe('PlanInfoBackendService', () => {
  let service: PlanInfoBackendService;
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getModelToken(PlanInfoModel),
          useValue: modelRepositoryMock,
        },
      ],
    }).compile();

    service = module.get<PlanInfoBackendService>(PlanInfoBackendService);
  });
