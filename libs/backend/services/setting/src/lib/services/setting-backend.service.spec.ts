...
import { SettingBackendService } from './setting-backend.service';
import {
  QueryOperatorEnum,
  SettingModel,
  SortByDirectionEnum,
...
describe('SettingBackendService', () => {
  let service: SettingBackendService;
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getModelToken(SettingModel),
          useValue: modelRepositoryMock,
        },
      ],
    }).compile();

    service = module.get<SettingBackendService>(SettingBackendService);
  });
